...
if __name__=="__main__":
    ...
    for selectedDataset in Datasets_To_test:
        if selectedDataset=="caltech101" :
            dataset = Caltech101Dataset(filter_label=None,images_path="/home/cap6412.student5/CTML/DinoDistilled/datasets/caltech101/101_ObjectCategories",preprocessin_fn=dinov1_model.dinov1_transform,subset="train",test_split=TEST_SPLIT_FOR_ZERO_SHOT_RETRIEVAL, random_seed=SEED_FOR_RANDOM_SPLIT)
            test_dataset = Caltech101Dataset(filter_label=None,images_path="/home/cap6412.student5/CTML/DinoDistilled/datasets/caltech101/101_ObjectCategories",preprocessin_fn=dinov1_model.dinov1_transform,subset="test",test_split=TEST_SPLIT_FOR_ZERO_SHOT_RETRIEVAL, random_seed=SEED_FOR_RANDOM_SPLIT)
        ...
        for query_idx, search_res in enumerate(I):
            ...
            test_intlabel = test_dataset.labels[query_idx][1]
            ...
            if test_strlabel not in class_scores["data"]:
                class_scores["data"][test_strlabel] = {"TP": 0, 
            ...
            for search_res_idx in search_res:
                intlabel = dataset.labels[search_res_idx][1]
            ...
            if test_strlabel in cosine_similarities_labels_str:
                class_scores["data"][test_strlabel]["TP"] +=1
                class_scores["data"][test_strlabel]["classIntanceRetrival"] +=classIntanceRetrival
                class_scores["data"][test_strlabel]["Predicted"].append(test_label)
            else:
                class_scores["data"][test_strlabel]["Predicted"].append(test_dataset.class_str_to_id[cosine_similarities_labels_str[0]])

                
            class_scores["data"][test_strlabel]["TotalRetrival"] +=TotalRetrival
            class_scores["data"][test_strlabel]["TotalClass"] +=1

            class_scores["data"][test_strlabel]["Topk"]["labels"].append(list(cosine_similarities_labels_str))
            class_scores["data"][test_strlabel]["Topk"]["scores"].append(list(cosine_similarities))
            class_scores["data"][test_strlabel]["Topk"]["images"].append(list(cosine_similarities_images))
            
            class_scores["data"][test_strlabel]["input_images"].append(originalImage)
            class_scores["data"][test_strlabel]["GroundTruths"].append(test_strlabel)

            TP  = class_scores["data"][test_strlabel]['TP']
            TotalClass = class_scores["data"][test_strlabel]['TotalClass']
            classIntanceRetrival = class_scores["data"][test_strlabel]['classIntanceRetrival']
            TotalRetrival = class_scores["data"][test_strlabel]['TotalRetrival']

            class_scores["data"][test_strlabel]["Recall"] = round(((TP*100)/TotalClass), 2)
            class_scores["data"][test_strlabel]["Precision"] = round(((classIntanceRetrival*100)/TotalRetrival), 2)
